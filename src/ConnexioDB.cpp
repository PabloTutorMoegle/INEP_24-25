...
void ConnexioDB::cosnultarUsuario(
    string sql) { // hacer que retorne en modo bool si existe o no
...
void ConnexioDB::modificaUsuario(string sql) {
    ...
    if (res->next()) {
        ...
        sql = "UPDATE Usuari SET nom = '" + nom_usuari +
              "', correu_electronic = '" + correu_electronic_usuari +
              "' WHERE sobrenom = '" + sobrenom_usuari + "'";
        stmt->execute(sql);
        cout << "----------" << "\n"
