...
BEGIN
    ...
    CREATE TABLE pelicula (
        ...
        pel_duracio INT UNSIGNED NOT NULL,
    ...
    CREATE TABLE visualitzacio_pelicula (
        vip_sobrenom_usuari varchar(200) ON DELETE CASCADE,
        vip_titol_pelicula varchar(200) ON DELETE CASCADE,
        vip_data DATETIME NOT NULL,
        vip_nb_visualitzacions INT UNSIGNED NOT NULL,

        FOREIGN KEY (vis_sobrenom_usuari) 
            REFERENCES usuari (usu_sobrenom),

        FOREIGN KEY (vis_titol_pelicula) 
            REFERENCES pelicula (pel_titol),

        CONSTRAINT chk_nb_visualitzacions 
            CHECK (vis_nb_visualitzacions > 0)
    );

    CREATE TABLE visualitzacio_capitol (
        vic_sobrenom_usuari varchar(200) ON DELETE CASCADE,
        vic_titol_serie varchar(200) ON DELETE CASCADE,
        vic_num_temporada INT UNSIGNED,
        vic_num_capitol INT UNSIGNED,
        vic_data DATETIME NOT NULL,
        vic_nb_visualitzacions INT UNSIGNED NOT NULL,
